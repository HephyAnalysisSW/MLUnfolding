...
max_values = np.max(train_data, keepdims=True, axis=0)
min_values = np.min(train_data, keepdims=True, axis=0)
...
for i in range(0, n_layers):
    transforms.append(MaskedPiecewiseRationalQuadraticAutoregressiveTransform(features=n_features, hidden_features=16, context_features=n_features_con))
    ...
    for i in range(num_epochs):
        ...
        for i_batch in range(max_batches):
            ...
            if i_batch % 500 == 0:
                ...
                print("memory : " + str(psutil.Process().memory_info().rss / (1024 * 1024 * 1024)) + "GB")
