...
argParser.add_argument('--plot_directory', action='store', default='EEEC_v2_173_5')
...
argParser.add_argument('--shuffle',  action='store', type=str, default="random")
argParser.add_argument('--split',    action='store', type=str, default="random") 
argParser.add_argument('--max_used_part',    action='store', type=int, default=50) 
argParser.add_argument('--pt_bin',    action='store', type=int, default=-1)
...
from EEEC.samples.nanoTuples_UL_RunII_nanoAOD_onesample import * # Careful Path to Files may be changed
    ...
    while r.run():
        ...
        if event.passSel and event.zeta_flag:
            for i in range(len(event.zeta_gen)):
                ...
                event_array[array_Count,zeta_rec_index] =    event.zeta_rec[i]
                event_array[array_Count,weight_gen_index] =  event.weight_gen[i]
                event_array[array_Count,zeta_gen_index] =    event.zeta_gen[i]
