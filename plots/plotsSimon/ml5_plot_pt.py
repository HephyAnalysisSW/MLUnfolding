...
for modelname in models:
    ...
    lower_border = lower_border *100
    step = (upper_border - lower_border) // number_of_bins
    n_bins = [x / 100.0 for x in range(lower_border,upper_border+1,step)] 
    
    hist1,bin_edges = np.histogram(retransformed_samples[:,pt_sample_index], bins= n_bins)
    hist2,_ = np.histogram(val_data[:,pt_rec_index]    ,bins= n_bins)
    hist3,_ = np.histogram(val_data[:,pt_gen_index] ,bins= n_bins)
    ...
    axs[0,0].set_xlim([200, 2000])
    axs[0,1].set_xlim([0, 0.0001])
    #axs[0,1].set_xlim([0, 0.00003])
    axs[0,2].set_xlim([0, 7])
  
    plt.savefig(plot_dir+"/data"+"/generated_data_"+modelname+"_pt.png")
    plt.close("all")
